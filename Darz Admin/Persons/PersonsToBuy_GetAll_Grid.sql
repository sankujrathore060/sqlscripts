CREATE PROCEDURE [dbo].[PersonsToBuy_GetAll_Grid]
	@PersonId INT = 0,
    @PageNo INT = 0,
    @PageSize INT = 10,
...
BEGIN
    ...
    SET @SQL = N'With CTE AS ( SELECT * FROM dbo.PersonsToBuy ma WITH(NOLOCK) ';

    SET @SQL = @SQL + N' WHERE PersonId=' + CAST(@PersonId AS NVARCHAR(10));
